...
BEGIN
  INSERT INTO auth.users (id, aud, role, email, created_at, updated_at)
  VALUES (v_uid, 'authenticated', 'authenticated',
          'race_breach_close_' || v_uid::TEXT || '@test.local', NOW(), NOW());
  ...
  UPDATE app_config SET value = 'false' WHERE key IN ('system_halt', 'system_readonly');
  UPDATE app_config SET value = 'true' WHERE key IN ('feature_futures_enabled', 'consent_gate_enabled');
  UPDATE rpc_rate_limit_configs SET capacity = 100, refill_rate = 100
   WHERE rpc_name IN ('rpc_open_futures_position', 'rpc_close_futures_position');
  INSERT INTO user_consents (user_id, doc_type, doc_version, accepted)
  SELECT v_uid, doc_type::consent_doc_type, 'test', TRUE
  FROM unnest(ARRAY['terms_of_service','privacy_policy','risk_disclosure','age_verification']) AS doc_type
  ON CONFLICT DO NOTHING;
  ...
  v_pos := rpc_open_futures_position(
    'PHONUSDT-PERP', 'long', 'USDT', '1000.000000', '10', NULL, NULL, 'race-breach-close-' || v_uid::TEXT
  );
  v_pos_id := (v_pos->>'position_id')::UUID;
  ...
  v_sweep := rpc_run_liquidations();
  ASSERT (v_sweep->>'liquidated')::INT = 0,
    'first breach sweep must not liquidate before user close: ' || v_sweep::TEXT;
  SELECT status INTO v_status FROM futures_positions WHERE id = v_pos_id;
  ASSERT v_status = 'open', 'position should remain open after first breach record';
  ...
  ASSERT v_after_close = v_before,
    format('USDT not conserved after close following first breach: before=%s after=%s', v_before, v_after_close);

  UPDATE oracle_prices SET price = '0.008900', updated_at = NOW()
   WHERE symbol = 'PHONUSDT-PERP';
  INSERT INTO price_ticks (symbol, price, created_at)
  ...
  ASSERT (v_sweep->>'liquidated')::INT = 0,
    'post-close sweep must ignore stale first breach state: ' || v_sweep::TEXT;
  ASSERT v_events = 1, 'expected exactly one settlement event after breach-close-sweep, got ' || v_events;
  ...
  ASSERT v_after_sweep = v_after_close,
    format('USDT changed after post-close breach sweep: after_close=%s after_sweep=%s',
           v_after_close, v_after_sweep);
  ASSERT NOT EXISTS (SELECT 1 FROM verify_ledger_hash_chain(v_uid)),
    'wallet ledger hash chain broken after breach-close-sweep race simulation';

  RAISE NOTICE 'SETTLEMENT BREACH-CLOSE RACE OK — first breach state ignored after user close';
END;
$$;
